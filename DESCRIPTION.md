...
*   **Метод `createOrder`**: Позволяет создавать новые заказы через протокол SOAP. Логика реализована в `SoapOrderService`.
...
### 2. Zero-Downtime переиндексация
...
### 4. Асинхронная обработка (Messenger)
Используется компонент Symfony Messenger для выноса тяжелых задач (отправка уведомлений, сложная индексация) в фон, чтобы не блокировать ответ пользователю.
